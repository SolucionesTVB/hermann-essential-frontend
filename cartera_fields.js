...
r._pagado=true;r._fecha_pago=fechaPago;if(window.actualizarClienteSupabase){await window.actualizarClienteSupabase(r.id,{pagado:true,desde:nuevaDesde,hasta:nuevaHasta});}if(window.cargarDatos){await window.cargarDatos();}render();alert('✅ Pago registrado\n\nPeriodo pagado: '+desdeConvertido+' - '+hastaActual+'\nNuevo periodo: '+nuevaDesde+' - '+nuevaHasta+(diasMora>0?'\n⚠️ Mora: '+diasMora+' días':''));}
...
function normMoneda(mon){const m=(mon||'CRC').toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();if(m.includes('dolar')||m==='usd'||m==='$'||m==='us')return 'USD';return 'CRC';}
function renderFila(r,isCartera){
  const m=normMoneda(r.moneda);
  const id=r.id||'';
  return COLS.map(c=>{
    if(c.key==='select')return '<td><input type="checkbox" class="cliente-checkbox" data-id="'+id+'" data-email="'+esc(r.correo||'')+'"></td>';
    if(c.key==='moneda'){const sym=m==='USD'?'$':'₡';return '<td style="font-size:13px;padding:10px 14px;white-space:nowrap;background:#ffffff;color:#1e293b;">'+sym+' '+m+'</td>';}
